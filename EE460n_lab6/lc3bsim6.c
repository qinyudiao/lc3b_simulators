...
void AGEX_stage() {
 ...
 if(mem_stall) LD_MEM=0;    // get LD_MEM
  ...
  if(ALUK == 0)
  	alures = Low16bits(sr1 + sr2);  //add
  else if(ALUK == 1)
  	alures = Low16bits(sr1 & sr2);  //and
  else if(ALUK == 2)
  	alures = Low16bits(sr1 ^ sr2);  //xor
  else if(ALUK == 3)                
  	alures = sr2;   //pass
...
void FETCH_stage() {
  ...
  /* your code for FETCH stage goes here */
   ...
   int de_v_temp = !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall & icache_r;
   //check stall
   if ((!mem_stall & !dep_stall & de_v_temp)   
