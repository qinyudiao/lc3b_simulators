...
int signextend(int input, int bit_in, int bit_out){
   if (input>>(bit_in-1) == 1) {
      int b = 1<<bit_in;
      int d = bit_out - bit_in;
      while (d-- > 0) {
         input += b;
         b = b << 1;
      }
...
2. 2-bit WE signal. WE0 is the write-enable for the the low byte of a word. WE1 is the write-enable for the high
byte of a word. You will need to generate these signals based on the values of DCACHE.R/W, DATA.SIZE, and
MEM.ADDRESS[0].
...
2. 16-bit output data. This is the data read from the D-Cache. Note that you need to design the logic to shift and sign
extend the appropriate byte if the access is a byte access.
*/
#define READ 1
#define WRITE 0
#define WORD 1
#define BYTE 0
int WE1,        //higher byte write-enable
    WE0,        //lower byte write-enable
    target_pc,
    trap_pc,
    v_mem_ld_cc,
    v_mem_ld_reg,
...
void MEM_stage() {
  ...
  // generate WE signals (2)
    ...
    else if(rw == WRITE){ // if write
        if(size == WORD){
            if(addr0 == 0){
                WE0 = 1;
                WE1 = 1;
            }
            else exit(-1); // unaligned access
        }
        else if(size == BYTE){
            WE0 = 1;
            WE1 = 0;
  ...
  // generate 1-bit enable signal (1)
  int dcache_enable = Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V;

  int dcache_ready = 0; //ready bit
  int data_out;         // 16bit output
  // 16-bit memory output
  if(dcache_enable) // memory is enabled
    dcache_access(PS.MEM_ADDRESS, &data_out, data_in, &dcache_ready, WE0, WE1);
  else data_out = 0x0000; // 
    mem_stall = dcache_enable & (!dcache_ready);

   if (size == BYTE) { 
      if (addr0)
        data_out = signextend(data_out >> 8, 8, 16);
      else 
        data_out = signextend(data_out & 0xFF, 8, 16);
   }
   trap_pc = data_out;
   ...
   v_mem_br_stall = PS.MEM_V & Get_MEM_BR_STALL(PS.MEM_CS);
    ...
    /* Generate 2-bit PCMUX for Fetch
   ...
   mem_pcmux = 0;
   if (PS.MEM_V) {
      if (Get_BR_OP(PS.MEM_CS)) {   // if is branch
         int n_ir = (PS.MEM_IR>>11) & 0x01;
         int z_ir = (PS.MEM_IR>>10) & 0x01;
         int p_ir = (PS.MEM_IR>>9) & 0x01;
         int n_mem = (PS.MEM_CC>>2) & 0x1;
         int z_mem = (PS.MEM_CC>>1) & 0x1;
         int p_mem = PS.MEM_CC & 0x01;
         int ben = (n_ir & n_mem) | (z_ir & z_mem) | (p_ir & p_mem);
         //printf("ben:%d \n", ben);
         if (ben) mem_pcmux = 1;
      }
      else if (Get_TRAP_OP(PS.MEM_CS))  //if is trap
         mem_pcmux = 2;
      else if (Get_UNCOND_OP(PS.MEM_CS))    //if is other control insts
         mem_pcmux = 1;
   }
...
void AGEX_stage() {
 ...
 if(mem_stall) LD_MEM=0;    // get LD_MEM
 else          LD_MEM=1;
  
  int addr1, addr2, address, sr2, alures;
  /* get address */
  if(Get_ADDR1MUX(PS.AGEX_CS) == 1)
  	addr1 = PS.AGEX_SR1;    // sr1
  else if(Get_ADDR1MUX(PS.AGEX_CS) == 0)
  	addr1 = PS.AGEX_NPC;    // next pc
  
  if(Get_ADDR2MUX(PS.AGEX_CS) == 0)
  	addr2 = 0;
  else if(Get_ADDR2MUX(PS.AGEX_CS) == 1)
    addr2 = signextend(PS.AGEX_IR & 0x3F, 6, 16);
  else if(Get_ADDR2MUX(PS.AGEX_CS) == 2)
    addr2 = signextend(PS.AGEX_IR & 0x1FF, 9, 16);
  else if(Get_ADDR2MUX(PS.AGEX_CS) == 3)
    addr2 = signextend(PS.AGEX_IR & 0x7FF, 11, 16);

  if(Get_LSHF1(PS.AGEX_CS)) 
    addr2 = addr2 << 1;
  if (Get_ADDRESSMUX(PS.AGEX_CS))
    address = Low16bits(addr1 + addr2);
  else address = (PS.AGEX_IR & 0xFF)<<1;

  /* ALU */
  if(Get_SR2MUX(PS.AGEX_CS) == 0)
  	sr2 = PS.AGEX_SR2;
  else if(Get_SR2MUX(PS.AGEX_CS) == 1)
  	sr2 = PS.AGEX_IR & 0x1F;
  
  int sr1 = PS.AGEX_SR1;
  int ALUK = Get_ALUK(PS.AGEX_CS);
  if(ALUK == 0)
  	alures = Low16bits(sr1 + sr2);
  else if(ALUK == 1)
  	alures = Low16bits(sr1 & sr2);
  else if(ALUK == 2)
  	alures = Low16bits(sr1 ^ sr2);
  ...
  int shfres;   
  int bit5 = (PS.AGEX_IR>>5) & 0x01;
  int bit4 = (PS.AGEX_IR>>4) & 0x01;
  int amount4 = PS.AGEX_IR & 0x0F;
  if (bit4 == 0) {
      ...
      shfres = PS.AGEX_SR1 >> amount4;
  if(Get_ALU_RESULTMUX(PS.AGEX_CS) == 0) // shift
      alures = shfres;
...
void DE_stage() {
  ...
  if(mem_stall) LD_AGEX=0;    // get LD_AGEX
  else          LD_AGEX=1;
  ...
  if (v_sr_ld_reg)
   ...
   if (PS.DE_V) {
      if (Get_SR1_NEEDED(DE_CS)) {
         if ((v_agex_ld_reg & (PS.AGEX_DRID == sr1)) 
            | (v_mem_ld_reg & (PS.MEM_DRID == sr1)) 
            | (v_sr_ld_reg & (PS.SR_DRID == sr1))){
            dep_stall = 1; 
         }
      }
      if (Get_SR2_NEEDED(DE_CS)) {
         if ((v_agex_ld_reg & (PS.AGEX_DRID == sr2)) 
            | (v_mem_ld_reg & (PS.MEM_DRID == sr2)) 
            | (v_sr_ld_reg & (PS.SR_DRID == sr2))){
            dep_stall = 1; 
         }
      }
...
void FETCH_stage() {
  ...
  /* your code for FETCH stage goes here */
   ...
   int data_out;
   icache_access(PC, &data_out, &icache_r);
   if ((!mem_stall & !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall & !dep_stall & icache_r) 
        | (v_mem_br_stall & (mem_pcmux!=0))){
      PC = pc_temp;
   }
   
   if (LD_DE) {
      NEW_PS.DE_NPC = pcp2_temp;
      NEW_PS.DE_IR = data_out;
