...
void readAndParse(FILE * file) {
    ...
    while (fgets(line, MAX_LINE_LENGTH+1, file) != NULL) {
      /* convert entire line to uppercase using toupper*/
      int i;
...
int isOpcode(char *str){
    if (belongs(str, OPCODES, OPCODE_NUMBER)) return 1;       //returns 1 if it is an opcode 
    if (belongs(str, PSEUDO_OPS, PSEUDO_OP_NUMBER)) return 0; //returns 0 if it is a peseudo_op
    return -1;                                              
}
...
char * toBinStringNonNeg(int dec, int bits) {
    /* check if the decimal is too large to represent */
