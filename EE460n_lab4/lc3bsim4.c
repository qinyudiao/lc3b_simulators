...
void initialize(char *ucode_filename, char *program_filename, int num_prog_files) { 
    ...
    CURRENT_LATCHES.SSP_R = 0x3000; /* Initial value of system stack pointer */
   ...
   */  
    ...
    if (GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        ...
        if(CURRENT_LATCHES.STATE_NUMBER != 15){ //do not check exception in Trap
            if(((NEXT_LATCHES.MAR < 0x3000) & temp_PSR_15) == 1){
                NEXT_LATCHES.E = 3; //protection
                //printf("PROTECTION EXCEPTION :%d ", NEXT_LATCHES.E);
            }
            else if((NEXT_LATCHES.MAR & 0x01) == 1){ //unaligned access
                //printf("UA EXCEPTION ");
                if((GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION) == 1) 
                                        | (CURRENT_LATCHES.STATE_NUMBER == 50) ){ //check if it is word
                    NEXT_LATCHES.E = 1; 
                }
            }
         }
    ...
    if(GetLD_EXCV(CURRENT_LATCHES.MICROINSTRUCTION) == 1){
        ...
        if(CURRENT_LATCHES.E == 0)   //Unknown opcode
            NEXT_LATCHES.EXCV = 4;
        if(CURRENT_LATCHES.E == 1)   //unaligned access
            NEXT_LATCHES.EXCV = 3;
